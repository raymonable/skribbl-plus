!function(t, e, n, o, r) {
    ...
    function c() {
        V.setItem("avatar", JSON.stringify(loginAvatarData))
    }
    function u(t, e) {
        t < 0 || t >= 3 || (e < 0 && (e = J[t] - 1),
        e >= J[t] && (e = 0),
    ...
    function l() {
        if (ct)
            if (at && at.drawingID == at.myID) {
                if (lt.drawCommands.length > 0) {
                    if (lt.drawCommands.length > 8) {
                        var t = lt.drawCommands.splice(7);
                        ct.emit("drawCommands", lt.drawCommands),
                        lt.drawCommands = t
                    } else
                        ct.emit("drawCommands", lt.drawCommands),
                        lt.drawCommands = []
                ...
                lt.drawCommands.length > 0 && (lt.drawCommands = [])
    }
    function p() {
        setTimeout(function() {
            n("#rateDrawing").show()
    ...
    function m() {
        ct && ct.emit("votekickCurrent"),
        y()
    }
    function g() {
        ht.goto("load");
        var t = null === V.getItem("firstLogin");
        t && V.setItem("firstLogin", "");
        var e = !1;
        if (!t) {
            var n = V.getItem("lastAd")
              , o = new Date;
    ...
    function C(t, e) {
        function handle_exit_btn(enabled) {
            document.querySelector('#exit_btn').setAttribute('data-original-title', !enabled ? 'Exit Scribbl' : 'Exit lobby');
            if (!enabled) {
                ut.playSound("playerLeave")
            }
        }
        ut.playSound("playerJoin"),
        ct = io(t, {
            query: {
                token: mt
            }
        }),
        window["game_instance"] = ct,
        ct.on("connect", function() {
            ct.on("disconnect", function() {
                ...
                at = null,
                ht.goto("login")
                handle_exit_btn(false)
                document.querySelector('#modalDisconnect').querySelector('.modal-title').innerHTML = 'Disconnected from server.';
                n("#modalDisconnect").modal()
            }),
            ct.emit("userData", e),
            ct.on("kicked", function() {
                n("#modalKicked").modal()
                handle_exit_btn(false)
            }),
            ct.on("drawCommands", function(t) {
                for (var e = 0; e < t.length; e++)
                    B(t[e])
            }),
            ct.on("canvasClear", function() {
                lt.clear()
            }),
            ct.on("lobbyKicked", function(t) {
                var e = at.players.get(t);
                null != e && (at.chatAddMsg(null, e.name + " was kicked!", G),
                e.kicked = !0)
            }),
            ct.on("chat", function(t) {
                var e = at.players.get(t.id)
                  , n = at.players.get(at.myID)
                  , o = e.id == at.drawingID || e.guessedWord;
                n.id != at.drawingID && !n.guessedWord && o || at.chatAddMsg(e, v(t.message), o ? "#7dad3f" : "#000")
            }),
            ct.on("lobbySpam", function() {
                at.chatAddMsg(null, "Spam detected! You're sending too many messages.", G)
            }),
            ct.on("restartNotification", function(t) {
                at && at.chatAddMsg(null, "Server will restart in about " + t + " seconds...", N)
            }),
            ct.on("lobbyVotekickCurrent", function(t) {
                if (at) {
                    var e = at.players.get(t[0])
                      , n = at.players.get(t[1]);
                    at.chatAddMsg(null, "'" + e.name + "' is voting to kick '" + n.name + "' (" + t[2] + "/" + t[3] + ")", N)
                }
            }),
            ct.on("lobbyConnected", function(_t) {
                var t = _t;
                t.players.forEach((player) => {
                    player.avatar[3] = 3;
                })
                handle_exit_btn(true)
                if (lt.clear(),
                E(""),
                at = new nt(t),
                at.inGame) {
                    for (var e = 0; e < t.drawCommands.length; e++)
                        B(t.drawCommands[e]);
                    ht.goto("game"),
                    at.drawingID >= 0 && D(at.time)
                } else
                    ht.goto("lobby")
            }),
            ct.on("lobbyLobby", function() {
                ht.goto("lobby")
            }),
            ct.on("lobbyState", function(t) {
                switch (parseInt(t[0])) {
                case $:
                    d(),
                    D(at.time);
            ...
            ct.on("lobbyDisconnected", function() {
                ct.emit("lobbyLeave"),
                at = null,
                ht.goto("login")
            }),
            ct.on("lobbyGameStart", function(t) {
                at.reset(),
                E(""),
                ht.goto("game")
            }),
            ct.on("lobbyGameEnd", function() {
                var t = [];
                at.players.forEach(function(e, n, o) {
                    t.push(e)
                }),
                L({
                    mode: "endgame",
            ...
            ct.on("lobbyLanguage", function(t) {
                at.setLanguage(t)
            }),
            ct.on("lobbyRounds", function(t) {
                at.setRounds(t)
            }),
            ct.on("lobbyDrawTime", function(t) {
                at.setDrawTime(t)
            }),
            ct.on("lobbyCustomWordsExclusive", function(t) {
                at.setCustomWordsExclusive(t)
            }),
            ct.on("lobbyPlayerConnected", function(_t) {
                var t = _t;
                t.avatar[3] = 3;
                at.addPlayer(t),
                at.chatAddMsg(null, t.name + " joined.", j),
                ut.playSound("playerJoin")
            }),
            ct.on("lobbyPlayerDisconnected", function(t) {
                var e = at.players.get(t);
                at.removePlayer(e),
                e.kicked || at.chatAddMsg(null, e.name + " left.", G),
                ut.playSound("playerLeave")
            }),
            ct.on("lobbyPlayerGuessedWord", function(t) {
                var e = at.players.get(t);
                at.setPlayerGuessed(e),
                at.chatAddMsg(null, e.name + " guessed the word!", _),
                t == at.myID && at.chatDisable(),
                ut.playSound("playerGuessed")
            }),
            ct.on("lobbyGuessClose", function(t) {
                at.chatAddMsg(null, "'" + t + "' is close!", N)
            }),
            ct.on("lobbyPlayerDrawing", function(t) {
                var e = at.players.get(t);
                at.setPlayerDrawing(e.id),
                at.chatAddMsg(null, e.name + " is drawing now!", O);
                var n = at.drawingID == at.myID;
                lt.setDrawing(n),
                n ? (f(),
                y()) : (p(),
                d()),
                ut.playSound("roundStart"),
                D(at.timeMax),
                U()
            }),
            ct.on("lobbyOwnerChanged", function(t) {
                var e = at.players.get(t);
                at.setPlayerOwner(e),
                at.chatAddMsg(null, e.name + " is the owner now!", _)
            }),
            ct.on("lobbyRound", function(t) {
                at.round = t,
                at.updateRound(),
                L({
                    mode: "text",
                    text: "Round " + (at.round + 1)
                })
            }),
            ct.on("lobbyChooseWord", function(t) {
                if (t.id == at.myID)
                    L({
                        mode: "choosewords",
                        text: "Choose a word",
                    ...
                    L({
                        mode: "text",
                        text: at.players.get(t.id).name + " is choosing a word!"
                    })
                }
            }),
            ct.on("lobbyReveal", function(t) {
                A(),
                at.chatEnable(),
                at.chatAddMsg(null, "The word was '" + t.word + "'", _);
                ...
                L({
                    ...
                    players: e,
                    onshow: function() {
                        n ? ut.playSound("roundEndFailure") : ut.playSound("roundEndSuccess")
                    }
                }),
                at.setPlayerDrawing(-1),
                lt.setDrawing(!1),
                at.playersResetGuessedWord()
            }),
            ct.on("lobbyPlayerRateDrawing", function(t) {
                var e = at.players.get(t[0])
                  , n = at.players.get(t[1])
                  , o = t[2];
                e && at.playerRate(e, o),
            ...
            ct.on("lobbyCurrentWord", function(t) {
                at.players.get(at.myID).guessedWord || E(t)
            }),
            ct.on("lobbyTime", function(t) {
                S(t)
            })
        })
    }
    function A() {
        null != gt && (clearInterval(gt),
        gt = null)
    }
    function D(t) {
        S(t),
    ...
    function T(t, e, n) {
        if (at.checkDrawing() && (lt.updateMousePosition(t, e, n),
        lt.brush.down)) {
            var o = null;
            switch (lt.brush.tool) {
            case "pen":
                o = lt.createDrawCommandLine(lt.brush.colorIndex, lt.brush.thickness, lt.mouseposPrev.x, lt.mouseposPrev.y, lt.mousepos.x, lt.mousepos.y);
                break;
            case "erase":
                o = lt.createDrawCommandErase(lt.brush.thickness, lt.mouseposPrev.x, lt.mouseposPrev.y, lt.mousepos.x, lt.mousepos.y);
                break;
            case "fill":
                lt.brush.toolUsed || (o = lt.createDrawCommandFill(lt.brush.colorIndex, lt.mouseposPrev.x, lt.mouseposPrev.y, lt.mousepos.x, lt.mousepos.y))
            }
            null != o && (lt.brush.toolUsed = !0,
            B(o))
        }
    }
    function I(t) {
    ...
    function B(t) {
        at.drawingID == at.myID ? (lt.drawCommands.push(t),
        lt.performDrawCommand(t)) : lt.addDrawCommandReceived(t)
    }
    function L(t) {
        if (ft)
            return pt.unshift(t),
                        ...
                        if (!n("json")) {
                            ...
                            !n("json-stringify")) {
                                var M = {
                                  ...
                                  , I = function(t, e, n, o, r, s, i) {
                                    var a, c, u, h, p, f, g, b, w, A, S, M, B, L, R, U;
                                    try {
                                        a = e[t]
                                    } catch (t) {}
                            ...
                            if (!n("json-parse")) {
                                var B, L, R = r.fromCharCode, U = {
                                    ...
                                    for (var t, e, n, o, r, s = L, i = s.length; B < i; )
                                        switch (r = s.charCodeAt(B)) {
                                        case 9:
                                        case 10:
                                        case 13:
                                        case 32:
                                        ...
                                        case 58:
                                        case 44:
                                            return t = A ? s.charAt(B) : s[B],
                                            B++,
                                            t;
                                        case 34:
                                            for (t = "@",
                                            B++; B < i; )
                                                if ((r = s.charCodeAt(B)) < 32)
                                                    _();
                                                else if (92 == r)
                                                    switch (r = s.charCodeAt(++B)) {
                                                    case 92:
                                                    ...
                                                    case 117:
                                                        for (e = ++B,
                                                        n = B + 4; B < n; B++)
                                                            (r = s.charCodeAt(B)) >= 48 && r <= 57 || r >= 97 && r <= 102 || r >= 65 && r <= 70 || _();
                                                        t += R("0x" + s.slice(e, B));
                                                        break;
                                                    default:
                                                        _()
                                                    ...
                                                    if (34 == r)
                                                        break;
                                                    for (r = s.charCodeAt(B),
                                                    e = B; r >= 32 && 92 != r && 34 != r; )
                                                        r = s.charCodeAt(++B);
                                                    t += s.slice(e, B)
                                                }
                                            if (34 == s.charCodeAt(B))
                                                return B++,
                                                t;
                                            _();
                                        default:
                                            ...
                                            r = s.charCodeAt(++B)),
                                            r >= 48 && r <= 57) {
                                                for (48 == r && (r = s.charCodeAt(B + 1)) >= 48 && r <= 57 && _(),
                                                o = !1; B < i && (r = s.charCodeAt(B)) >= 48 && r <= 57; B++)
                                                    ;
                                                if (46 == s.charCodeAt(B)) {
                                                    for (n = ++B; n < i && (r = s.charCodeAt(n)) >= 48 && r <= 57; n++)
                                                        ;
                                                    n == B && _(),
                                                    B = n
                                                }
                                                if (101 == (r = s.charCodeAt(B)) || 69 == r) {
                                                    for (r = s.charCodeAt(++B),
                                                    43 != r && 45 != r || B++,
                                                    n = B; n < i && (r = s.charCodeAt(n)) >= 48 && r <= 57; n++)
                                                        ;
                                                    n == B && _(),
                                                    B = n
                                                }
                                            ...
                                            "true" == s.slice(B, B + 4))
                                                return B += 4,
                                                !0;
                                            if ("false" == s.slice(B, B + 5))
                                                return B += 5,
                                                !1;
                                            if ("null" == s.slice(B, B + 4))
                                                return B += 4,
                                                null;
                                            _()
                                        }
    ...
    const _ = "#56CE27"
      ...
      , $ = 4;
    var K = !1;
    if (void 0 !== t.localStorage)
        try {
            t.localStorage.setItem("test", "test"),
            K = "test" == t.localStorage.getItem("test")
        } catch (t) {
            console.log("window.localStorage not accessible!"),
            console.log(t)
        }
    var V;
    K ? (V = t.localStorage,
    console.log("window.localStorage is accessible.")) : V = {
    ...
    var Z = function(t, e) {
      ...
      , Q = function() {
        ...
        this.sounds = new Map,
        this.mute = !1,
        "true" === V.getItem("audioMute") && this.setMute(!0),
        t.addEventListener("load", this.load.bind(this), !1)
    };
    Q.prototype.setMute = function(t) {
        this.mute = t,
        this.mute ? n("#audio").css("background-image", "url(res/audio_off.gif") : n("#audio").css("background-image", "url(res/audio.gif"),
        V.setItem("audioMute", this.mute.toString())
    }
    ,
    Q.prototype.addSound = function(t, e) {
        this.sounds.set(e, new Z(this,t))
    }
    ,
    Q.prototype.loadSounds = function() {
    ...
    ,
    Q.prototype.playSound = function(t) {
    ...
    tt.prototype.updateMousePosition = function(t, e, n) {
    ...
    ,
    tt.prototype.addDrawCommandReceived = function(t) {
        this.drawCommandsReceived.push(t)
    }
    ,
    tt.prototype.addDrawCommand = function(t) {
        this.drawCommands.push(t)
    }
    ,
    tt.prototype.createDrawCommandLine = function(t, e, n, o, r, s) {
        return [0, t, e, n, o, r, s]
    }
    ,
    tt.prototype.createDrawCommandErase = function(t, e, n, o, r) {
        return [1, t, e, n, o, r]
    }
    ,
    tt.prototype.createDrawCommandFill = function(t, e, n) {
        return [2, t, e, n]
    }
    ,
    tt.prototype.performDrawCommand = function(t) {
        switch (t[0]) {
        case 0:
    ...
    ,
    tt.prototype.getImageData = function(t, e, n, o) {
        var r = Math.min(t, n)
          , s = Math.min(e, o)
          , i = Math.abs(t - n)
          , a = Math.abs(e - o);
    ...
    ,
    tt.prototype.setPixel = function(t, e, n, o, r) {
    ...
    ,
    tt.prototype.getPixel = function(t, e, n) {
        var o = 4 * (n * t.width + e);
        return o >= 0 && o < t.data.length ? [t.data[o], t.data[o + 1], t.data[o + 2]] : [0, 0, 0]
    }
    ,
    tt.prototype.floodFill = function(t, e, n, o, r) {
    ...
    ,
    tt.prototype.plotLineOld = function(t, e, n, o, r, s) {
    ...
    ,
    tt.prototype.plotLine = function(t, e, n, o, r, s, i, a) {
    ...
    ,
    tt.prototype.setDrawing = function(t) {
        t ? (this.brush.show(),
    ...
    ,
    et.prototype.setDown = function(t) {
        this.down = t,
        this.down || (this.toolUsed = !1),
        n("#cursor .tool").css("top", this.down ? 0 : -8)
    }
    ,
    et.prototype.setTool = function(t) {
        this.tool = t,
    ...
    ,
    et.prototype.updateBrushCursor = function() {
        switch (this.tool) {
        case "pen":
        case "erase":
            if (this.thickness <= 0)
    ...
    nt.prototype.updatePlayerAvatars = function() {
        var t = this;
    ...
    ,
    nt.prototype.checkDrawing = function() {
        return this.drawingID == this.myID
    }
    ,
    nt.prototype.addPlayer = function(t) {
    ...
    ,
    nt.prototype.removePlayer = function(t) {
    ...
    ,
    nt.prototype.playersResetGuessedWord = function() {
    ...
    ,
    nt.prototype.setPlayerOwner = function(t) {
        this.ownerID = t.id,
        this.containerGamePlayersUpdateOwner(),
        this.containerLobbyPlayersUpdateOwner()
    }
    ,
    nt.prototype.setPlayerDrawing = function(t) {
        this.drawingID = t,
        this.containerGamePlayersUpdateDrawing()
    }
    ,
    nt.prototype.setPlayerGuessed = function(t) {
        t.guessedWord = !0,
        this.containerGamePlayersUpdateGuessed()
    }
    ,
    nt.prototype.setPlayerScore = function(t, e, n) {
        t.score = e,
        t.scoreGuessed = n,
    ...
    ,
    nt.prototype.setRounds = function(t) {
        this.roundMax = t,
        this.updateRound(),
        this.containerLobbyUpdateRounds()
    }
    ,
    nt.prototype.setCustomWordsExclusive = function(t) {
        this.useCustomWordsExclusive = !!t,
        this.containerLobbyUpdateUseCustomWordsExclusive()
    }
    ,
    nt.prototype.setDrawTime = function(t) {
        this.timeMax = t,
        this.containerLobbyUpdateDrawTime()
    }
    ,
    nt.prototype.updateRound = function() {
        n("#round").text("Round " + (this.round + 1) + " of " + this.roundMax)
    }
    ,
    nt.prototype.updateRanks = function() {
        var t = [];
        this.players.forEach(function(e, n, o) {
            t.push(e)
        }),
    ...
    ,
    nt.prototype.containerGamePlayerClear = function() {
        this.containerGame.empty()
    }
    ,
    nt.prototype.containerGamePlayerGet = function(t) {
        return this.containerGame.find("#player" + t.id)
    }
    ,
    nt.prototype.containerGamePlayerAdd = function(t) {
    ...
    ,
    nt.prototype.containerGamePlayerRemove = function(t) {
        this.containerGamePlayerGet(t).remove()
    }
    ,
    nt.prototype.containerGamePlayersUpdateGuessed = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    ,
    nt.prototype.containerGamePlayersUpdateDrawing = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    ,
    nt.prototype.containerGamePlayersUpdateOwner = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    ,
    nt.prototype.containerGamePlayerUpdateScore = function(t) {
        this.containerGamePlayerGet(t).find(".score").text("Points: " + t.score)
    }
    ,
    nt.prototype.containerGamePlayerUpdateRank = function(t, e) {
        this.containerGamePlayerGet(t).find(".rank").text("#" + e)
    }
    ,
    nt.prototype.containerLobbyPlayerGet = function(t) {
        return this.containerLobby.find("#player" + t.id)
    }
    ,
    nt.prototype.containerLobbyPlayerAdd = function(t) {
    ...
    ,
    nt.prototype.containerLobbyPlayerRemove = function(t) {
        this.containerLobbyPlayerGet(t).remove()
    }
    ,
    nt.prototype.containerLobbyPlayersUpdateOwner = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    ,
    nt.prototype.containerLobbyPlayerClear = function() {
        this.containerLobby.empty()
    }
    ,
    nt.prototype.containerLobbyUpdateRounds = function() {
        n("#lobbySetRounds").val(this.roundMax)
    }
    ,
    nt.prototype.containerLobbyUpdateDrawTime = function() {
        n("#lobbySetDrawTime").val(this.timeMax)
    }
    ,
    nt.prototype.containerLobbyUpdateUseCustomWordsExclusive = function() {
        n("#lobbyCustomWordsExclusive").attr("checked", !!this.useCustomWordsExclusive)
    }
    ,
    nt.prototype.containerLobbyUpdateLanguage = function() {
        var t = M(this.language);
        n("#lobbySetLanguage").val(t)
    }
    ,
    nt.prototype.playerMessage = function(t, e) {
    ...
    ,
    nt.prototype.playerRate = function(t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        e > 0 ? n.find(".iconThumbsUp").show() : n.find(".iconThumbsDown").show()
    }
    ,
    nt.prototype.playerMessageHandle = function(t) {
    ...
    ,
    nt.prototype.chatClear = function() {
        n("#boxMessages").empty()
    }
    ,
    nt.prototype.chatAddMsg = function(t, e, o) {
        var r = n("#boxMessages")
    ...
    ,
    nt.prototype.chatDisable = function() {}
    ,
    nt.prototype.chatEnable = function() {}
    ;
    var ot = function(t, e) {
        this.element = t,
        this.logo = e,
    ...
    var rt = function() {
        this.screens = {
            load: new ot(n("#screenLoading"),0),
            login: new ot(n("#screenLogin"),2),
            lobby: new ot(n("#screenLobby"),1),
            game: new ot(n("#screenGame"),1)
        },
        this.screenCurrent = null
    };
    rt.prototype.goto = function(t) {
    ...
    ,
    rt.prototype.hideLogo = function() {
        n("#containerLogoBig").hide(),
        n("#containerLogoSmall").hide()
    }
    ,
    rt.prototype.showLogo = function(t) {
    ...
    ;
    var st = t.location != t.parent.location ? e.referrer : e.location.href
      , it = function(t, e) {
        return -1 !== t.indexOf(e)
    };
    (it(st, "skribbl-io.com") || it(st, "skribblio.com") || it(st, "skribbl-io.org") || it(st, "skribblio.online")) && (t.top.location.href = "https://www.skribbl.io"),
    adplayer = null,
    ...
    var at = null
      ...
      , ut = new Q
      , ht = new rt
      , lt = new tt(n("#canvasGame"))
      , pt = new Array
      , ft = !1
      , dt = !1;
    window["screen_manager"] = ht;
    lt.setDrawing(!1),
    ht.goto("login");
    var yt = !1;
    ht.screens.game.onshow = function() {
        yt || (yt = !0,
    ...
    n("#inputName").change(function() {
        V.setItem("name", n("#inputName").val())
    }),
    null != V.getItem("name") && n("#inputName").val(V.getItem("name")),
    n("#loginLanguage").change(function() {
        V.setItem("lang", n("#loginLanguage").val())
    }),
    null != V.getItem("lang") && n("#loginLanguage").val(V.getItem("lang")),
    loginAvatarData = function() {
        var t = V.getItem("avatar");
        return t ? (() => {t = JSON.parse(t); t[3] = 3; return t;})() : (t = [Math.round(Math.random() * J[0]), Math.round(Math.random() * J[1]), Math.round(Math.random() * J[2]), 3],
        V.setItem("avatar", JSON.stringify(t))),
        t
    }(),
    ...
    n("#rateDrawing .thumbsUp").on("click", function() {
        ct && ct.emit("rateDrawing", 1),
        f()
    }),
    n("#rateDrawing .thumbsDown").on("click", function() {
        ct && ct.emit("rateDrawing", 0),
        f()
    }),
    f(),
    y(),
    ...
    n("#lobbySetLanguage").on("change", function() {
        ct && at.myID == at.ownerID && ct.emit("lobbySetLanguage", n(this).val())
    }),
    n("#lobbySetRounds").on("change", function() {
        ct && at.myID == at.ownerID && ct.emit("lobbySetRounds", n(this).val())
    }),
    n("#lobbySetDrawTime").on("change", function() {
        ct && at.myID == at.ownerID && ct.emit("lobbySetDrawTime", n(this).val())
    }),
    n("#lobbyCustomWordsExclusive:checkbox").change(function() {
        if (ct && at.myID == at.ownerID) {
            var t = this.checked;
            ct.emit("lobbySetCustomWordsExclusive", t)
        }
    }),
    ...
    n("#buttonLobbyPlay").on("click", function() {
        ct && at.myID == at.ownerID && ct.emit("lobbyGameStart", n("#lobbySetCustomWords").val())
    }),
    n("#formChat").submit(function(t) {
        var e = n("#inputChat")
          , o = e.val();
        return o && (ct ? ct.emit("chat", o) : at.chatAddMsg(at.players.get(at.myID), o, "#000"),
        e.val("")),
        !1
    }),
    ...
    n("#buttonLobbyCreate").on("click", function() {
        ...
        ct.emit("lobbyCreate", t),
        n("#modalCreateLobby").modal("hide")
    }),
    n("#buttonRefreshLobbies").on("click", function() {
        ct.emit("requestLobbies")
    }),
    lt.canvas.on("mousewheel DOMMouseScroll", function(t) {
        t.preventDefault();
        var e = t.originalEvent.wheelDelta > 0 || t.originalEvent.detail < 0 ? 1 : -1;
        lt.brush.setThickness(lt.brush.thickness + 6 * e)
    }),
    n(e).on("mousemove", function(t) {
        at && at.checkDrawing() && T(t.clientX, t.clientY, !1)
    }),
    n(e).keypress(function(t) {
        if (at && at.checkDrawing() && t !== r && t.key !== r)
            switch (t.key.toUpperCase()) {
            case "B":
                lt.brush.setTool("pen");
                break;
            case "E":
                lt.brush.setTool("erase");
                break;
            case "F":
                lt.brush.setTool("fill")
            }
    }),
    lt.canvas.on("mousedown", function(t) {
        switch (t.preventDefault(),
        t.button) {
        case 0:
            lt.brush.down || (lt.brush.setDown(!0),
            T(t.clientX, t.clientY, !0))
        }
    });
    var bt = null;
    /*lt.canvas.on("touchstart", function(t) {
        t.preventDefault();
        var e = t.changedTouches;
        e.length > 0 && null == bt && (bt = e[0].identitfier,
        lt.brush.setDown(!0),
        T(e[0].clientX, e[0].clientY, !0))
    }),
    lt.canvas.on("touchend", function(t) {
        t.preventDefault(),
        bt = null,
        lt.brush.setDown(!1)
    }),
    lt.canvas.on("touchcancel", function(t) {
        t.preventDefault(),
        bt = null,
        lt.brush.setDown(!1)
    }),
    lt.canvas.on("touchmove", function(t) {
        t.preventDefault();
        for (var e = t.changedTouches, n = 0; n < e.length; n++)
            if (e[n].identitfier == bt) {
    ...
    n("#buttonClearCanvas").on("click", function() {
        ct ? ct.emit("canvasClear") : lt.clear()
    }),
    n(".containerTools .tool").on("click", function() {
        lt.brush.setTool(n(this).data("tool"))
    }),
    n(".colorItem").on("click", function() {
        var t = n(this).data("color");
        lt.brush.setColor(Number(t))
    }),
    n(".brushSize").on("click", function() {
        var t = lt.brush.thicknessMin
          , e = lt.brush.thicknessMax
          , o = Number(n(this).data("size")) * (e - t) + t;
        lt.brush.setThickness(o)
    })
    window["submit_chat"] = () => {
            var e = n("#inputChat")
              , o = e.val();
